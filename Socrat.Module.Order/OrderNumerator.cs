...
namespace Socrat.Module.Order
{
    ...
    /// </summary>
    public static class OrderNumerator
    {
        public static string GetNext()
        {
            ...
            {
                Logger.AddWarning($"Счетчик номеров заявок выдал повторяющийся номер {_number}");
                _number = GenerateNumber();
            }
        ...
        {
            bool res = true;
            using (Socrat.Core.IRepository<Socrat.Core.Entities.Order> _repo = DataHelper.GetRepository<Socrat.Core.Entities.Order>())
            {
