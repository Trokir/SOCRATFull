...
namespace Socrat.Core
{
    ...
    public abstract class PropertyChangedBase : INotifyPropertyChanged
    {
        ...
        public void SetParentsChanged(bool state)
        {
            IEntity _this = this as IEntity;
            if (_this != null && _this.ParentEntities != null && _this.ParentEntities.Exists(x => x != null && !x.Changed))
                foreach (IEntity entity in _this.ParentEntities.Where(x => x != null && !x.Changed))
                    entity.Changed = state;
        }
        ...
        private void SetHostedEntitiesChangeValue(bool state)
        {
            ...
            IEnumerable<FieldInfo> _fields =
                this.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Instance).ToList()
                    .Where(x => x.GetCustomAttribute(typeof(ParentItemAttribute)) == null)
                    .Where(x => x.FieldType.GetInterfaces().Contains(typeof(IEntity)));
            IEntity _entity;
            ...
            _fields =
                this.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Instance).ToList()
                    .Where(x => x.GetCustomAttribute(typeof(ParentItemAttribute)) == null)
                    .Where(x => x.FieldType.GetInterfaces().Contains(typeof(IList)));
