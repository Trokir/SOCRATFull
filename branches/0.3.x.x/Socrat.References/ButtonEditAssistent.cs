...
{
    public class ButtonEditAssistent<T1, T2, T3> : INotifyPropertyChanged
...

    {
        ...
        private Expression<Func<T1, bool>> _externalFilterExp;
        /// <summary>
        /// Условие внешней фильтрации
        /// </summary>
        public Expression<Func<T1, bool>> ExternalFilterExp
        {
            get => _externalFilterExp;
            set => SetExternalFilterExp(value);
        }

        public Func<T2> SlelectionFormContructorDependencesInjection { get; set; }
        public Func<T3> EditorFormContructorDependencesInjection { get; set; }


        private void SetExternalFilterExp(Expression<Func<T1, bool>> value)
        {
            _externalFilterExp = value;
        }

        public ButtonEditAssistent(ButtonEdit editor, T1 entity, Action<ITabable, DialogOutputType> onDialogOutput)
        {
            ButtonEdit = editor;
            _value = entity;
        ...
        {
            T1 _tmp = ButtonEdit.EditValue as T1;
            if (_tmp != null && (_value == null || (_value.Id != _tmp.Id)))
            {
                _value = _tmp;
        ...
        private void SetClearButtonVisible()
        {
            if (_editorButtons != null)
                _editorButtons.FirstOrDefault(x => x.Kind == ButtonPredefines.Clear).Visible = ButtonEdit.EditValue != null;
        }
        ...
        {
            ISelectionDialogFilterable<T1> _fx = GetSelectionForm();
            _fx.SetSingleSelectMode(_value);
            _fx.ItemSelected += (sender, args) =>
            {
                _value = _fx.SelectedItem as T1;
                if (ButtonEdit != null)
        ...
        private ISelectionDialogFilterable<T1> GetSelectionForm()
        {
            if (SlelectionFormContructorDependencesInjection != null)
                return SlelectionFormContructorDependencesInjection.Invoke();
        ...
        {
            if (EditorFormContructorDependencesInjection != null)
                EditorFormContructorDependencesInjection.Invoke();
        ...
        private void _notifyObj_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                _value = Obj.GetType().GetProperty(ObjProperty).GetValue(Obj) as T1;
                ButtonEdit.EditValue = _value;
