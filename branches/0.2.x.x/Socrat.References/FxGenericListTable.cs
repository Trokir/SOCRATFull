...
{
    public partial class FxGenericListTable<T> : FxBaseForm, ISelectionDialog where T : class, IEntity, new()
    {
        ...
        public FxGenericListTable()
        {
            ...
            this.DataBindings.Add("Text", this, "Title", false, DataSourceUpdateMode.OnPropertyChanged);
        }
        ...
        private void OnLoad(object sender, EventArgs e)
        {
            using (EFDataFactory _factory = new EFDataFactory())
                Repository = _factory.CreateRepository<IRepository<T>>();
            RefreshData();
        }
        ...
        {
            Type type = typeof(T);
            var prop = type.GetProperty(fieldName + "_Id");
            if (prop != null)
