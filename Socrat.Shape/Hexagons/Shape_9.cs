...
{
    sealed class Shape_9 : Hexagon
    {
        public Shape_9(List<ShapePoint> ShapePoints, List<dynamic> currentShapeParametersList) : base(ShapePoints, currentShapeParametersList)
        {
        ...
        public override void GetShapeComponents()
        {
            ...
            using (Pen pen = new Pen(Color.Blue, SizeLineBoundArgument))
            {
                ...
                ShapePoint h1s = GetNewCustomPoint(SetPointCurrentValueX(A) + 20 * LineBoundArgument, SetPointCurrentValueY(B));
                ShapePoint h1e = GetNewCustomPoint(SetPointCurrentValueX(A) + 20 * LineBoundArgument, SetPointCurrentValueY(A));
                Line l1h = GetNewLine(h1s, h1e);
                ShapePoint h1center = GetNewCustomPoint(SetPointCurrentValueX(A) + 20 * LineBoundArgument, ACheck.PointY - l1h.Length / 2);
                ...
                ShapePoint le = GetNewCustomPoint(SetPointCurrentValueX(E), Z_Base.PointY + 40 * LineBoundArgument);
                Line ll = GetNewLine(ls, le);
                ShapePoint lcenter = GetNewCustomPoint(SetPointCurrentValueX(A) + (ll.Length / 2), Z_Base.PointY + 40 * LineBoundArgument);
                ...
                ShapePoint l2s = GetNewCustomPoint(SetPointCurrentValueX(D), W_Base.PointY + 10 * LineBoundArgument);
                ShapePoint l2e = GetNewCustomPoint(SetPointCurrentValueX(E), W_Base.PointY + 10 * LineBoundArgument);
                Line ll2 = GetNewLine(l2s, l2e);
                ShapePoint l2center = GetNewCustomPoint(SetPointCurrentValueX(D) + ll2.Length / 2, W_Base.PointY + 15 * LineBoundArgument);
                ...
                ShapePoint b3s = GetNewCustomPoint(SetPointCurrentValueX(E), Z_Base.PointY + 40 * LineBoundArgument);
                ...
                ShapePoint hs = GetNewCustomPoint(Y_Base.PointX + 20 * LineBoundArgument, SetPointCurrentValueY(D));
                ShapePoint he = GetNewCustomPoint(Z_Base.PointX + 20 * LineBoundArgument, SetPointCurrentValueY(F));
                Line lh = GetNewLine(hs, he);
                ShapePoint hcenter = GetNewCustomPoint(Y_Base.PointX + 22 * LineBoundArgument, SetPointCurrentValueY(F) - lh.Length / 2);
                graphicsShape.DrawLine(pen, hs, he);
                graphicsShape.DrawString("H=" + SetCurrentSize(SetH), drawFontBold, Brushes.Black, hcenter, sf);

                ShapePoint hs2 = GetNewCustomPoint(Y_Base.PointX + 10 * LineBoundArgument, SetPointCurrentValueY(E));
                ShapePoint he2 = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, SetPointCurrentValueY(F));
                Line lh2 = GetNewLine(hs2, he2);
                ShapePoint h2center = GetNewCustomPoint(Y_Base.PointX + 12 * LineBoundArgument, SetPointCurrentValueY(F) - lh2.Length / 2);
                ...
                if (Area > 0.012)
                {
                    ...
                    graphicsShape.DrawString("9", drawNumbertBold, Brushes.Black, mCustCenter, sf);
                }
                using (Pen pens = new Pen(Color.Blue, SizeLineBoundArgument / 2))
                {
                    ...
                    graphicsShape.DrawLine(pens, hs2, SetPointCurrentType(E));
        ...
        public override double SetH2 { get => Math.Round(E_line.Length, 0); set => base.SetH2 = value; }
        ...
        public override double SetH_t
        {
            get
            {
                ShapePoint Sh = GetNewCustomPoint(ACheck.PointX, CCheck.PointY);
        ...
        {
            get => Math.Round(E_Check_Line.Length, 0);
        }
        public override double SetH1_t
        {
            get => Math.Round(A_Check_Line.Length, 0);
        }
        public override double SetL_t { get => Math.Round(F_Check_Line.Length, 0); }
        public override double SetL1_t
        {
            get
            {
                ...
                Line l = GetNewLine(CurvePoint, CCheck);
                return Math.Round(l.Length, 0);
            }
        }
        public override double SetL2_t
        {
            get
            {
                ...
                Line l = GetNewLine(CurvePoint, DCheck);
                return Math.Round(l.Length, 0);
            }
        }
        ...
        protected override void SetLValue()
        {
            ...
            double diff = F.PointX - TempPoint.PointX;
            D.PointX -= diff;
        ...
        {
            base.SetH2Value();
            E.PointY = SetCurrentLineLength(F, E, _SetH2).PointY;
            ValidValue = false;
        }
