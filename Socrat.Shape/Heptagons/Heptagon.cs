...
namespace Socrat.Shape
{
    ...
    /// семиугольник
    /// </summary>
    public partial class Heptagon : BaseShape
    {
        private bool _IsDrawHeptagonByAngleValue;
        ...
        private double _SetResizeAGsideValue;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Heptagon"/> class.
        /// </summary>
        /// <param name="Core.Entities.ShapePoints">The custom points.</param>
        public Heptagon(List<ShapePoint> ShapePoints, List<dynamic> currentShapeParametersList) : base(ShapePoints, currentShapeParametersList)
        {
           // SelectedSides=new int[] {0,0,0,0,0,0,0 };
            ShapePoint[] points = new ShapePoint[] { A = GetNewPoint(), B = GetNewPoint(), C = GetNewPoint(), D = GetNewPoint(), E = GetNewPoint(), F = GetNewPoint(), G = GetNewPoint() };
            try
            {
            ...
            G_Check_Line = GetNewLine(GCheck, ACheck);
        ...
        public override void SelectClickedSide(int xCoord, int yCoord, bool flag)
        {
            ...
            {
                if (flag) { ColorMarker6 = "rowCheckCut6";SelectedSides.SetValue(6, 5); ClickedSelectSide = 6; SelectedSidesLength += F_line.Length; }
                else { ColorMarker6 = ""; SelectedSides.SetValue(0, 5); ClickedSelectSide = 0; SelectedSidesLength -= F_line.Length; }
            }
            if (ThicknessPath(G, A).IsVisible(point))
            {
                if (flag) { ColorMarker7 = "rowCheckCut7"; SelectedSides.SetValue(7, 6); ClickedSelectSide = 7; SelectedSidesLength += G_line.Length; }
                else { ColorMarker7 = ""; SelectedSides.SetValue(0, 6); ClickedSelectSide = 0; SelectedSidesLength -= G_line.Length; }
            }
            else return;
        }
        ...
        public override double Perimeter
        {
            get
            {
                double AArc = SetA_radius * Math.PI * (CalculateAngle(G, A, B) <= 90 ? CalculateAngle(F, A, B) : 180 - CalculateAngle(F, A, B)) / 180;
                ...
                double GArc = SetG_radius * Math.PI * (CalculateAngle(F, G, A) <= 90 ? CalculateAngle(F, G, A) : 180 - CalculateAngle(F, G, A)) / 180;
                Line a = new Line(A2, B2);
                Line b = new Line(B1, C1);
                Line c = new Line(C2, D1);
                Line d = new Line(D2, E1);
                Line e = new Line(E2, F1);
                Line f = new Line(F2, G2);
                Line g = new Line(G1, A1);
                return Math.Round(AArc + BArc + CArc + DArc + EArc + FArc + GArc + a.Length + b.Length + c.Length + d.Length + e.Length + f.Length + g.Length, 1)/1000;
            }
        }
        ...
        public override double Area
        {
            get
            {
                ...
                double AAngle = (CalculateAngle(G, A, B) <= 90 ? CalculateAngle(G, A, B) : 180 - CalculateAngle(G, A, B)) / 180;
                ...
                Line Fa = new Line(E2, E);
                Line Fb = new Line(E1, E);
                Line Fc = new Line(E1, E2);
                ...
                Line Ga = new Line(G2, G);
                Line Gb = new Line(G1, G);
                Line Gc = new Line(G1, G2);
                double GAngle = (CalculateAngle(F, G, A) <= 90 ? CalculateAngle(F, G, A) : 180 - CalculateAngle(F, G, A)) / 180;
                double GTriangle = Math.Round(Math.Sqrt(((Fa.Length + Fb.Length + Fc.Length) / 2) *
                   (((Ga.Length + Gb.Length + Gc.Length) / 2) - Ga.Length) *
                   (((Ga.Length + Gb.Length + Gc.Length) / 2) - Gb.Length) *
                   (((Ga.Length + Gb.Length + Gc.Length) / 2) - Gc.Length)), 2) -
                 (Math.Pow(SetG_radius, 2) / 2) * (Math.PI * GAngle * Math.PI / 180 - Math.Sin(GAngle * Math.PI / 180));
                double baseSquare = 0.5 * Math.Abs((A.PointX * B.PointY + B.PointX * C.PointY + C.PointX * D.PointY + D.PointX * E.PointY + E.PointX * F.PointY + F.PointX * G.PointY + G.PointX * A.PointY) - (B.PointX * A.PointY + C.PointX * B.PointY + D.PointX * C.PointY + E.PointX * D.PointY + F.PointX * E.PointY + G.PointX * F.PointY + A.PointX * G.PointY));
                return Math.Round((baseSquare - Math.Abs(ATriangle) - Math.Abs(BTriangle)
                    - Math.Abs(CTriangle)
                    - Math.Abs(DTriangle) - Math.Abs(ETriangle) - Math.Abs(FTriangle) - Math.Abs(GTriangle))/1000000,3);
            }
        }
        ...
        public override double TrueArea
        {
            get
            {
                ...
                double baseSquare = 0.5 * Math.Abs((ACheck.PointX * BCheck.PointY + BCheck.PointX * CCheck.PointY + CCheck.PointX * DCheck.PointY +
                    DCheck.PointX * ECheck.PointY + ECheck.PointX * FCheck.PointY + FCheck.PointX * GCheck.PointY + GCheck.PointX * ACheck.PointY) - (BCheck.PointX * ACheck.PointY + CCheck.PointX *
                    BCheck.PointY + DCheck.PointX * CCheck.PointY + ECheck.PointX * DCheck.PointY + FCheck.PointX * ECheck.PointY + GCheck.PointX * FCheck.PointY + ACheck.PointX * GCheck.PointY));
        ...
        /// <summary>
        /// Gets or sets the set resize a gside value.
        /// </summary>
        /// <value>
        /// The set resize a gside value.
        /// </value>
        [DisplayName("Сторона GA -H ")]
        [Category("Стороны")]
        [Display(GroupName = "Изменение  размеров ")]
        public double SetResizeAGsideValue
        {
            get { return Math.Round(G_line.Length, 0); }
            set
            {
                SetField(ref _SetResizeAGsideValue, value, () => SetResizeAGsideValue);
                G.PointX = SetCurrentLineLength(A, G, _SetResizeAGsideValue).PointX;
                G.PointY = SetCurrentLineLength(A, G, _SetResizeAGsideValue).PointY;
        ...
        public bool IsDrawHeptagonByAngleValue
        {
            get { return _IsDrawHeptagonByAngleValue; }
            set
            {
                SetField(ref _IsDrawHeptagonByAngleValue, value, () => IsDrawHeptagonByAngleValue);
                if (!_IsDrawHeptagonByAngleValue) return;
                else
                {
                    G.PointY = A.PointY;
                    G.PointX = SetCurrentLineLength(A, G, G_line.Length).PointX;
                    G.PointY = SetCurrentLineLength(A, G, G_line.Length).PointY;
                    ...
                    IsDrawHeptagonByAngleValue = false;
                }
            }
        }
        ...
        {
            double angle = Math.Abs(CalculateAngle(F, G, A));
            CenterPoint.PointX = (A.PointX + B.PointX + C.PointX + D.PointX + E.PointX + F.PointX + G.PointX) / 7;
            CenterPoint.PointY = (A.PointY + B.PointY + C.PointY + D.PointY + E.PointY + F.PointY + G.PointY) / 7;
            ShapePoint[] points = new ShapePoint[] { A, B, C, D, E, F, G };
        ...
        protected override void AllowanceProcessing()
        {
            ...
            diag1 = (diag1 <= 90) ? _CheckCut1 / Math.Sin(CalculateAngle(G, A, B) * Math.PI / 180) :
                _CheckCut1 /(90 - ((180 -  Math.Sin(CalculateAngle(G, A, B)) * Math.PI / 180)));
            ...
            diag61 = (diag61 <= 90) ? _CheckCut6 / Math.Sin(CalculateAngle(F, G, A) * Math.PI / 180) :
              _CheckCut6 /(90 - ((180 -  Math.Sin(CalculateAngle(F, G, A)) * Math.PI / 180)));

            diag7 = (diag7 <= 90) ? _CheckCut7 / Math.Sin(CalculateAngle(F, G, A) * Math.PI / 180) :
             _CheckCut7 /(90 - ((180 -  Math.Sin(CalculateAngle(F, G, A)) * Math.PI / 180)));
            diag71 = (diag71 <= 90) ? _CheckCut7 / Math.Sin(CalculateAngle(G, A, B) * Math.PI / 180) :
              _CheckCut7 /(90 - ((180 -  Math.Sin(CalculateAngle(G, A, B)) * Math.PI / 180)));

          

            ACheck.PointY = SetCurrentLineLength(GCheck, ACheck, diag1 + G_Check_Line.Length).PointY;
            ACheck.PointX = SetCurrentLineLength(GCheck, ACheck, diag1 + G_Check_Line.Length).PointX;
            ...
            GCheck.PointY = SetCurrentLineLength(ACheck, GCheck, diag61 + G_Check_Line.Length).PointY;
            GCheck.PointX = SetCurrentLineLength(ACheck, GCheck, diag61 + G_Check_Line.Length).PointX;
            ...
            ACheck.PointY = SetCurrentLineLength(BCheck, ACheck, diag71 + A_Check_Line.Length).PointY;
            ACheck.PointX = SetCurrentLineLength(BCheck, ACheck, diag71 + A_Check_Line.Length).PointX;

            if (IsToothVector == true)
            {
                //using (Pen penCut = new Pen(Color.Red, ThiсknessArgument))
                //{
                //    PointF[] cutPoints = new PointF[] { ACheck, BCheck, CCheck, DCheck, ECheck, FCheck, GCheck};
                ...
                {
                    graphicsShape.DrawLine(pen7, GCheck, ACheck);
                }
                IsToothVector = true;
            }
            else
        ...
        public override List<ShapePoint> ShapePointsList()
        {
            ...
            List<ShapePoint> ShapePoints = new List<ShapePoint>() { A, B, C, D, E, F,G };
            return ShapePoints;
        }
        /// <summary>
        /// Converts to string.
        ...
        {
            string points = string.Format("A {0} , B {1} , C {2} \n, D {3},E{4} ,F{5},G{6} \n", A.ToString(), B.ToString(), C.ToString(), D.ToString(), E.ToString(), F.ToString(), G.ToString());
            string lines = string.Format(" AB ={0} ; BC={1} ; CD ={2} ;\n DE={3} ; EF={4}; FG={5}; GA={6}\n ", A_line.Length, B_line.Length, C_line.Length, D_line.Length, E_line.Length, F_line.Length, G_line.Length);
            string otherParameters = string.Format(" Периметр = {0} ; Площадь ={1}\n ", this.Perimeter, this.Area);
            string otherParameters1 = string.Format(" Угол FAB = {0},Угол ABC = {1},Угол BCD = {2},\nУгол CDE = {3} ,Угол DEF = {4} ,Угол EFG= {5} ,Угол FGA= {6}  \n ",
                CalculateAngle(F, A, B), CalculateAngle(A, B, C), CalculateAngle(B, C, D), CalculateAngle(C, D, E), CalculateAngle(D, E, F), CalculateAngle(E, F, G), CalculateAngle(F, G, A));
