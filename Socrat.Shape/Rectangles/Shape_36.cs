...
{
    sealed class Shape_36 : Rectangular
    {
        public Shape_36(List<ShapePoint> ShapePoints, List<dynamic> currentShapeParametersList) : base(ShapePoints, currentShapeParametersList)
        {
        ...
        public override void GetShapeComponents()
        {
            ...
            using (Pen pen = new Pen(Color.Blue, SizeLineBoundArgument))
            {
                ...
                ShapePoint hs = GetNewCustomPoint(Y_Base.PointX + 20 * LineBoundArgument, C.PointY);
                ShapePoint he = GetNewCustomPoint(Z_Base.PointX + 20 * LineBoundArgument, D.PointY);
                Line lh = GetNewLine(hs, he);
                ShapePoint hcenter = GetNewCustomPoint(Y_Base.PointX + 20 * LineBoundArgument, D.PointY - lh.Length / 2);
                graphicsShape.DrawLine(pen, hs, he);
                sf.FormatFlags = StringFormatFlags.LineLimit;
                graphicsShape.DrawString("H=" + SetH, drawFontBold, Brushes.Black, hcenter, sf);


                ShapePoint b2s = GetNewCustomPoint(Y_Base.PointX + 20 * LineBoundArgument, C.PointY);
                ...
                ShapePoint l1s = GetNewCustomPoint(C.PointX, W_Base.PointY + 10 * LineBoundArgument);
                ShapePoint l1e = GetNewCustomPoint(D.PointX, W_Base.PointY + 10 * LineBoundArgument);
                graphicsShape.DrawLine(pen, l1s, l1e);
                Line l1h = GetNewLine(l1s, l1e);
                ShapePoint l1scenter = GetNewCustomPoint(C.PointX + l1h.Length / 2, W_Base.PointY + 10 * LineBoundArgument);
                ...
                ShapePoint b21scenter = GetNewCustomPoint(Z_Base.PointX + 20 * LineBoundArgument, Z_Base.PointY - b2h1.Length / 2);
                graphicsShape.DrawString("B2=" + SetB2, drawFontBold, Brushes.Black, b21scenter, sf);

                ShapePoint ls = GetNewCustomPoint(B.PointX, W_Base.PointY + 40 * LineBoundArgument);
                ShapePoint le = GetNewCustomPoint(D.PointX, W_Base.PointY + 40 * LineBoundArgument);
                Line ll = GetNewLine(ls, le);
                ShapePoint lcenter = GetNewCustomPoint(B.PointX + (ll.Length / 2), W_Base.PointY + 40 * LineBoundArgument);
                graphicsShape.DrawLine(pen, ls, le);
                graphicsShape.DrawString("L=" + SetL, drawFontBold, Brushes.Black, lcenter, sf);

                ShapePoint b3s = GetNewCustomPoint(W_Base.PointX, W_Base.PointY + 40 * LineBoundArgument);
                ShapePoint b3e = GetNewCustomPoint(B.PointX, W_Base.PointY + 40 * LineBoundArgument);
                ...
                ShapePoint b1s = GetNewCustomPoint(D.PointX, W_Base.PointY + 40 * LineBoundArgument);
                ShapePoint b1e = GetNewCustomPoint(Z_Base.PointX, W_Base.PointY + 40 * LineBoundArgument);
                Line lb1 = GetNewLine(b1s, b1e);
                ShapePoint b1center = GetNewCustomPoint(Z_Base.PointX - (lb1.Length / 2), W_Base.PointY + 20 * LineBoundArgument);
                ...
                if (Area > 0.012)
                {
                    ...
                    graphicsShape.DrawString("36", drawNumbertBold, Brushes.Black, mCustCenter, sf);
                }
                using (Pen pens = new Pen(Color.Blue, SizeLineBoundArgument / 2))
                {
                    ...
                    graphicsShape.DrawLine(pens, l1e, D);
                    graphicsShape.DrawLine(pens, ls, B);
                    ...
                    {
                        graphicsShape.DrawLine(pens1, l1s, C);
                    }
                }
            }
            #endregion
        ...
        public override double SetL { get => Math.Round(GetNewLine(GetNewCustomPoint(D.PointX, C.PointY), B).Length, 0); set => base.SetL = value; }
        public override double SetL1 { get => Math.Round(GetNewLine(GetNewCustomPoint(D.PointX, C.PointY), C).Length, 0); set => base.SetL1 = value; }
        ...
        protected override void SetL1Value()
        {
            ...
            TempPoint.PointX = D.PointX + 0.001;
            TempPoint.PointY = C.PointY;
            C.PointX = SetCurrentLineLength(TempPoint, C, _SetL1).PointX - 0.001;
            TempPoint.PointX = B.PointX - 0.001;
            TempPoint.PointY = A.PointY;
            A.PointX = SetCurrentLineLength(TempPoint, A, _SetL1).PointX + 0.001;
            var upLine = GetNewLine(B, C);
            B_downFault.PointX = SetCurrentLineLength(B, B_downFault, upLine.Length / 2).PointX;
            var downLine = GetNewLine(D, A);
            A_downFault.PointX = SetCurrentLineLength(D, A_downFault, downLine.Length / 2).PointX;
            ValidValue = false;
        }
