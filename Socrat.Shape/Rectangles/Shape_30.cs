...
{
    sealed class Shape_30 : Rectangular
    {
        public Shape_30(List<ShapePoint> ShapePoints, List<dynamic> currentShapeParametersList) : base(ShapePoints, currentShapeParametersList) { }
        ...
        public override void GetShapeComponents()
        {
            ...
            using (Pen pen = new Pen(Color.Blue, SizeLineBoundArgument))
            {
                ...
                ShapePoint hs = GetNewCustomPoint(Y_Base.PointX + 40 * LineBoundArgument, BCheck.PointY);
                ...
                ShapePoint h2s = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, BCheck.PointY);
                ShapePoint h2e = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, A_downFault.PointY);
                graphicsShape.DrawLine(pen, h2s, h2e);
                Line b2h = GetNewLine(h2s, h2e);
                ShapePoint h2scenter = GetNewCustomPoint(Z_Base.PointX + 15 * LineBoundArgument, A_downFault.PointY - b2h.Length / 2);
                Line A_up = GetNewLine(B, A_upFault);
                graphicsShape.DrawString("H2=" + (A_line.Length - (A_line.Length - A_up.Length)), drawFontBold, Brushes.Black, h2scenter, sf);

                ShapePoint h1s = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, B_upFault.PointY);
                ShapePoint h1e = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, DCheck.PointY);
                Line lh1 = GetNewLine(h1s, h1e);
                ShapePoint h1center = GetNewCustomPoint(Z_Base.PointX + 15 * LineBoundArgument, DCheck.PointY - (lh1.Length / 2));
                graphicsShape.DrawLine(pen, h1s, h1e);
                    Line B_up = GetNewLine(D, B_upFault);
                    graphicsShape.DrawString("H1=" + (B_line.Length - (B_line.Length - B_up.Length)), drawFontBold, Brushes.Black, h1center, sf);
                ...
                if (Area > 0.012)
                {
                    ...
                    graphicsShape.DrawString("30", drawNumbertBold, Brushes.Black, mCustCenter, sf);
                }
                using (Pen pens = new Pen(Color.Blue, SizeLineBoundArgument / 2))
                {
                    graphicsShape.DrawLine(pens, h1s, B_downFault);
                    ...
                    {
                        graphicsShape.DrawLine(pens1, h2e, A_upFault);
                    }
                }
        ...
        public override double SetH1_FaultLine_To_RightSide
        {
            get
            {
                Line l = GetNewLine(D, B_downFault);
                return Math.Round(l.Length, 0);
            }
            set
            {
                TempValue = SetH1_FaultLine_To_RightSide;
                SetField(ref _SetH1_FaultLine_To_RightSide, value, () => SetH1_FaultLine_To_RightSide);
                if (!CheckValidSize()) SetH1Value();
                else
                {
                    SetH1_FaultLine_To_RightSide = TempValue;
                    return;
                }
            }
        }
        public override double SetH2_FaultLine_To_LeftSide
        {
            get
            {
                Line l = GetNewLine(B, A_downFault);
                return Math.Round(l.Length, 0);
            }
            set
            {
                TempValue = SetH2_FaultLine_To_LeftSide;
                SetField(ref _SetH2_FaultLine_To_LeftSide, value, () => SetH2_FaultLine_To_LeftSide);
                if (!CheckValidSize()) SetH2Value();
                else
                {
                    SetH2_FaultLine_To_LeftSide = TempValue;
                    return;
                }
            }
        }
        ...
        public override bool CheckValidSize()
        {
            ...
            if (_SetH < 0 || _SetH1 < 0 || _SetL < 0 || _SetB1 < 0 || _SetB2 < 0 || _SetB3 < 0|| _SetH1_FaultLine_To_RightSide<0|| _SetH2_FaultLine_To_LeftSide<0)
            {
            ...
            else if ((_SetH > 0) && _SetH < SetH1_FaultLine_To_RightSide)
            {
                ValidValue = true;
                ValidateSetSizeMessage(Text: "Значение H не может быть меньше H1");
            }
            else if ((_SetH1_FaultLine_To_RightSide > 0) && _SetH1_FaultLine_To_RightSide > SetH)
            {
                ValidValue = true;
                ValidateSetSizeMessage(Text: "Значение H1 не может быть больше H");
            }
            else if ((_SetH > 0) && _SetH < SetH2_FaultLine_To_LeftSide)
            {
                ValidValue = true;
                ValidateSetSizeMessage(Text: "Значение H не может быть меньше H2");
            }
            else if (_SetH <= 0 || _SetH >= SetH1_FaultLine_To_RightSide + SetH2_FaultLine_To_LeftSide)
            {
                if ((_SetH2_FaultLine_To_LeftSide > 0) && _SetH2_FaultLine_To_LeftSide > SetH)
                {
                    ValidValue = true;
        ...
        public override void AddCustomProperties(object sender, CustomPropertyDescriptorsEventArgs e)
        {
            ...
            if (e.Context.PropertyDescriptor == null)
            {
                ...
                AddIfPropertyExist(e.Properties, filteredCollection, "SetH2_FaultLine_To_LeftSide");
                AddIfPropertyExist(e.Properties, filteredCollection, "SetH1_FaultLine_To_RightSide");
