...
{
    sealed class Shape_38 : Rectangular
    {
        public Shape_38(List<ShapePoint> ShapePoints, List<dynamic> currentShapeParametersList) : base(ShapePoints, currentShapeParametersList)
        {
        ...
        public override void GetShapeComponents()
        {
            ...
            using (Pen pen = new Pen(Color.Blue, SizeLineBoundArgument))
            {
                ...
                ShapePoint hcenter = GetNewCustomPoint(Y_Base.PointX + 20 * LineBoundArgument, SetPointCurrentValueY(D) - lh.Length / 2);
                ...
                ShapePoint l1e = GetNewCustomPoint(SetPointCurrentValueX(D), W_Base.PointY + 10 * LineBoundArgument);
                ...
                ShapePoint lcenter = GetNewCustomPoint(SetPointCurrentValueX(B) + (ll.Length / 2), W_Base.PointY + 30 * LineBoundArgument);
                ...
                if (Area > 0.012)
                {
                    ...
                    graphicsShape.DrawString("38", drawNumbertBold, Brushes.Black, mCustCenter, sf);
                }
                using (Pen pens = new Pen(Color.Blue, SizeLineBoundArgument / 2))
                {
                    ...
                    graphicsShape.DrawLine(pens, le, l1e);
                    graphicsShape.DrawLine(pens, b1e, Z_Base);
                    graphicsShape.DrawLine(pens, b3s, W_Base);
                    graphicsShape.DrawLine(pens, b3e, SetPointCurrentType(B));
        ...
        protected override void SetHValue()
        {
            base.SetHValue();
            var pr = B.PointY;
            ShapePoint p = GetNewCustomPoint(D.PointX, C.PointY);
            D.PointY = SetCurrentLineLength(p, D, _SetH).PointY;
            A.PointY = D.PointY;
