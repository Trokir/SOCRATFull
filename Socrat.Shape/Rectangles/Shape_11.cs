...
{
    sealed class Shape_11 : Rectangular
    {
        public Shape_11(List<ShapePoint> ShapePoints, List<dynamic> currentShapeParametersList) : base(ShapePoints, currentShapeParametersList) 
        {
        ...
        public override void GetShapeComponents()
        {
            ...
            using (Pen pen = new Pen(Color.Black, SizeLineBoundArgument))
            {
                ...
                MoveInternalLineFromLeft(graphicsShape, W_Base, X_Base, First_B_Some_Size_Point, Second_B_Some_Size_Point, SetB1, SetB4);
                FindPointDrawLine(SetPointCurrentType(B), SetPointCurrentType(C), SetB4, SetB1, SetB2);
            }
            using (Pen pen = new Pen(Color.Blue, SizeLineBoundArgument))
            {
                ...
                ShapePoint hcenter = GetNewCustomPoint(Y_Base.PointX + 40 * LineBoundArgument, SetPointCurrentValueY(D) - lh.Length / 2);
                graphicsShape.DrawLine(pen, hs, he);
                    graphicsShape.DrawString("H=" + SetCurrentSize(SetH), drawFontBold, Brushes.Black, hcenter, sf);

                ShapePoint b4s = GetNewCustomPoint(Y_Base.PointX + 10 * LineBoundArgument, Y_Base.PointY);
                ShapePoint b4e = GetNewCustomPoint(Y_Base.PointX + 10 * LineBoundArgument, Y_Base.PointY + SetB4);
                graphicsShape.DrawLine(pen, b4s, b4e);
                Line b4h = GetNewLine(b4s, b4e);
                ShapePoint b4scenter = GetNewCustomPoint(Y_Base.PointX + 10 * LineBoundArgument, Y_Base.PointY + b4h.Length / 2);
                graphicsShape.DrawString("B4=" + SetB4, drawFontBold, Brushes.Black, b4scenter, sf);

                ShapePoint b4s2 = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, Z_Base.PointY);
                ShapePoint b4e2 = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, Z_Base.PointY - SetB4);
                ...
                ShapePoint b4s1 = GetNewCustomPoint(W_Base.PointX, W_Base.PointY + 10 * LineBoundArgument);
                ShapePoint b4e1 = GetNewCustomPoint(W_Base.PointX + SetB4, W_Base.PointY + 10 * LineBoundArgument);
                graphicsShape.DrawLine(pen, b4s1, b4e1);
                Line b4h1 = GetNewLine(b4s1, b4e1);
                ShapePoint b41scenter = GetNewCustomPoint(W_Base.PointX, W_Base.PointY + 10 * LineBoundArgument);
                ...
                ShapePoint b2e = GetNewCustomPoint(Y_Base.PointX + 40 * LineBoundArgument, X_Base.PointY);
                graphicsShape.DrawLine(pen, b2s, b2e);
                Line b2h = GetNewLine(b2s, b2e);
                ShapePoint b2scenter = GetNewCustomPoint(Y_Base.PointX + 40 * LineBoundArgument, X_Base.PointY + b2h.Length / 2);
                graphicsShape.DrawString("B2=" + SetB2, drawFontBold, Brushes.Black, b2scenter, sf);


                ShapePoint b3s = GetNewCustomPoint(SetPointCurrentValueX(D), Z_Base.PointY + 40 * LineBoundArgument);
                ShapePoint b3e = GetNewCustomPoint(Z_Base.PointX, Z_Base.PointY + 40 * LineBoundArgument);
                ...
                ShapePoint b1s = GetNewCustomPoint(SetPointCurrentValueX(B), W_Base.PointY + 40 * LineBoundArgument);
                ...
                if (Area > 0.012)
                {
                    ...
                    graphicsShape.DrawString("11", drawNumbertBold, Brushes.Black, mCustCenter, sf);
                }
                using (Pen pens = new Pen(Color.Blue, SizeLineBoundArgument / 2))
                {
                    ...
                    ShapePoint wer2 = GetNewCustomPoint(SetPointCurrentValueX(D), W_Base.PointY);
                    ShapePoint wer3 = GetNewCustomPoint(SetPointCurrentValueX(D), X_Base.PointY);
                    ShapePoint wer4 = GetNewCustomPoint(Y_Base.PointX, Y_Base.PointY+SetB4);
                    ...
                    graphicsShape.DrawLine(pens, b4e, wer4);
        ...
        protected override void FindPointDrawLine(ShapePoint firsrPoint, ShapePoint secondPoint, double factor = 0,
            double anotherFactor = 0, double thofactor = 0)
        {
            ...
            bnP.PointY = secondPoint.PointY + (firsrPoint.PointY - secondPoint.PointY) * (length / Math.Sqrt(Math.Pow((firsrPoint.PointX - secondPoint.PointX), 2) + Math.Pow((firsrPoint.PointY - secondPoint.PointY), 2)));
        ...
        public override double SetH { get => Math.Round(GetNewLine(GetNewCustomPoint(D.PointX, B.PointY), D).Length, 0); set => base.SetH = value; }
        ...
        public override double SetH_t {
            get
            {
                ShapePoint d = GetNewCustomPoint(DCheck.PointX, BCheck.PointY);
                Line l = GetNewLine(d, DCheck);
                return l.Length;
            }
        }
        public override double SetH1_t { get => Math.Round(C_Check_Line.Length, 0); }
        public override double SetL_t {
            get
            {
                ShapePoint sp = GetNewCustomPoint(BCheck.PointX, ACheck.PointY);
                Line l = GetNewLine(sp, DCheck);
        ...
        protected override void SetLValue()
        {
            base.SetLValue();
            D.PointY = A.PointY;
            ShapePoint Sp = GetNewCustomPoint(B.PointX, A.PointY);
            TempPoint.PointX = SetCurrentLineLength(Sp, D, _SetL).PointX;
        ...
        public override bool CheckValidSize()
        {
            ...
            var height = _SetH + diag + _SetB4+_SetB2 + value4;
