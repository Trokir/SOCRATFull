...
{
    sealed class Shape_17 : Rectangular
    {
        public Shape_17(List<ShapePoint> ShapePoints, List<dynamic> currentShapeParametersList) : base(ShapePoints, currentShapeParametersList)
        {
            var fdf = B.PointY;
            var ere = B.PointX;
        }
        ...
        public override void GetShapeComponents()
        {
            ...
            using (Pen pen = new Pen(Color.Black, SizeLineBoundArgument))
            {
                ...
                MoveInternalLineFromRight(graphicsShape, Y_Base, Z_Base, First_B_Some_Size_Point, Second_B_Some_Size_Point, SetB1, SetB3);
            ...
            using (Pen pen = new Pen(Color.Blue, SizeLineBoundArgument))
            {
                ...
                ShapePoint hs = GetNewCustomPoint(Y_Base.PointX + 30 * LineBoundArgument, SetPointCurrentValueY(B));
                ShapePoint he = GetNewCustomPoint(Y_Base.PointX + 30 * LineBoundArgument, SetPointCurrentValueY(D));
                Line lh = GetNewLine(hs, he);
                ShapePoint hcenter = GetNewCustomPoint(Y_Base.PointX + 35 * LineBoundArgument, SetPointCurrentValueY(B) + lh.Length / 2);
                ...
                ShapePoint h1center = GetNewCustomPoint(Y_Base.PointX + 12 * LineBoundArgument, SetPointCurrentValueY(C) + (lh1.Length / 2));
                ...
                ShapePoint b2s = GetNewCustomPoint(Y_Base.PointX + 10 * LineBoundArgument, SetPointCurrentValueY(B));
                ShapePoint b2e = GetNewCustomPoint(Y_Base.PointX + 10 * LineBoundArgument, Y_Base.PointY);
                graphicsShape.DrawLine(pen, b2s, b2e);
                Line b2h = GetNewLine(b2s, b2e);
                ShapePoint b2scenter = GetNewCustomPoint(Y_Base.PointX + 10 * LineBoundArgument, X_Base.PointY + b2h.Length / 2);
                graphicsShape.DrawString("B2=" + SetB2, drawFontBold, Brushes.Black, b2scenter, sf);



                ShapePoint b3s1 = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, SetPointCurrentValueY(A));
                ShapePoint b3e1 = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, W_Base.PointY);
                Line lb31 = GetNewLine(b3s1, b3e1);
                ShapePoint b31center = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, W_Base.PointY - (lb31.Length / 2));
                graphicsShape.DrawLine(pen, b3s1, b3e1);
                graphicsShape.DrawString("B3=" + SetB3, drawFontBold, Brushes.Black, b31center, sf);


                ShapePoint b3s2 = GetNewCustomPoint(SetPointCurrentValueX(A), W_Base.PointY + 40 * LineBoundArgument);
                ShapePoint b3e2 = GetNewCustomPoint(W_Base.PointX, W_Base.PointY + 40 * LineBoundArgument);
                Line lb32 = GetNewLine(b3s2, b3e2);
                ShapePoint b32center = GetNewCustomPoint(W_Base.PointX + (lb32.Length / 2), W_Base.PointY + 40);
                graphicsShape.DrawLine(pen, b3s2, b3e2);
                graphicsShape.DrawString("B3=" + SetB3, drawFontBold, Brushes.Black, b32center, sf);



                ShapePoint b1s = GetNewCustomPoint(SetPointCurrentValueX(D), Z_Base.PointY + 40 * LineBoundArgument);
                ShapePoint b1e = GetNewCustomPoint(Z_Base.PointX, Z_Base.PointY + 40 * LineBoundArgument);
                Line lb1 = GetNewLine(b1s, b1e);
                ShapePoint b1center = GetNewCustomPoint(Z_Base.PointX - (lb1.Length / 2), Z_Base.PointY + 40 * LineBoundArgument);
                ...
                ShapePoint b3s = GetNewCustomPoint(Y_Base.PointX, Z_Base.PointY + 15 * LineBoundArgument);
                ShapePoint b3e = GetNewCustomPoint(Y_Base.PointX - SetB3, Z_Base.PointY + 15 * LineBoundArgument);
                Line lb3 = GetNewLine(b3s, b3e);
                ShapePoint b3center = GetNewCustomPoint(Y_Base.PointX - (lb3.Length / 2), Z_Base.PointY + 15 * LineBoundArgument);
                graphicsShape.DrawLine(pen, b3s, b3e);
                graphicsShape.DrawString("B3=" + SetB3, drawFontBold, Brushes.Black, b3center, sf);


                ShapePoint ls1 = GetNewCustomPoint(SetPointCurrentValueX(B), Z_Base.PointY + 15 * LineBoundArgument);
                ShapePoint le1 = GetNewCustomPoint(SetPointCurrentValueX(C), Z_Base.PointY + 15 * LineBoundArgument);
                Line ll1 = GetNewLine(ls1, le1);
                ShapePoint l1center = GetNewCustomPoint(SetPointCurrentValueX(B) + (ll1.Length / 2), Z_Base.PointY + 15 * LineBoundArgument);
                ...
                if (Area > 0.012)
                {
                    ...
                    graphicsShape.DrawString("17", drawNumbertBold, Brushes.Black, mCustCenter, sf);
                }

                using (Pen pens = new Pen(Color.Blue, SizeLineBoundArgument / 2))
                {
                    ShapePoint wer = GetNewCustomPoint(SetPointCurrentValueX(A), W_Base.PointY);
                    ShapePoint wer1 = GetNewCustomPoint(Z_Base.PointX, SetPointCurrentValueY(A));
                    ShapePoint wer2 = GetNewCustomPoint(SetPointCurrentValueX(B), X_Base.PointY);
                    ...
                    graphicsShape.DrawLine(pens, b3s1, wer1);
                    graphicsShape.DrawLine(pens, b3e2, W_Base);
                    graphicsShape.DrawLine(pens, ls1, wer);
                    graphicsShape.DrawLine(pens, he, wer1);
                    graphicsShape.DrawLine(pens, hs, b2s);
        ...
        protected override void FindPointDrawLine(ShapePoint firsrPoint, ShapePoint secondPoint, double factor = 0, double anotherFactor = 0, double thofactor = 0)
        {
            ...
            point.PointX = D.PointX + anotherFactor - factor;
            ...
            ShapePoint SPoint = GetNewPoint();
            SPoint = SetCurrentLineLength(firsrPoint, secondPoint, length);
            if (SetB1 > 0)
            {
                using (Pen pen = new Pen(Color.Black, ThiсknessArgument / 2))
                {
                    graphicsShape.DrawLine(pen, secondPoint, SPoint);
                }
            }
        ...
        public override double SetH1 { get => Math.Round(GetNewLine(D, GetNewCustomPoint(D.PointX, C.PointY)).Length, 0); set => base.SetH1 = value; }
        ...
        public override double SetH1_t
        {
            get
            {
                ShapePoint s = GetNewCustomPoint(DCheck.PointX, CCheck.PointY);
                Line l = GetNewLine(DCheck, s);
        ...
        {
            base.SetHValue();
          //  Move(0, _SetH + CheckCut2 + CheckCut3);
        ...
        {
            var baseAngle = CalculateAngle(A, B, D);
            Line Diagonal = GetNewLine(B, D);
            ShapePoint Sa = GetNewCustomPoint(C.PointX, B.PointY);
            C.PointX = SetCurrentLineLength(B, Sa, _SetL1).PointX;
            var currentAngle = CalculateAngle(A, B, C);
            ValidValue = false;
        }
        protected override void SetH1Value()
        {
            base.SetH1Value();
            ShapePoint s = GetNewCustomPoint(D.PointX, C.PointY);
            C.PointY = SetCurrentLineLength(D, s, _SetH1).PointY;
            ValidValue = false;
        }
        ...
        {
            var l = GetNewLine(B, D);
            var p = GetNewCustomPoint(C.PointX, A.PointY);
            var line = GetNewLine(p, C);
            double diag = 0;
            diag = (diag <= 90)
