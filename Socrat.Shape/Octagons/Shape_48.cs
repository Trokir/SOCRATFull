...
{
    sealed class Shape_48 : Octagon
    {
        public Shape_48(List<ShapePoint> ShapePoints, List<dynamic> currentShapeParametersList) : base(ShapePoints, currentShapeParametersList)
        {

        }
        public override double Perimeter =>
            Math.Round((A_line.Length + B_line.Length + C_line.Length +
                        D_line.Length + E_line.Length + F_line.Length + G_line.Length + H_line.Length) / 1000, 3);
        public override double Perimeter_t =>
            Math.Round((A_Check_Line.Length + B_Check_Line.Length + C_Check_Line.Length + D_Check_Line.Length +
        ...
        public override void GetShapeComponents()
        {
            ...
            using (Pen pen = new Pen(Color.Blue, SizeLineBoundArgument))
            {
                ...
                ShapePoint b3s = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, X_Base.PointY);
                ShapePoint b3e = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, X_Base.PointY + SetB2);
                graphicsShape.DrawLine(pen, b3s, b3e);
                Line b3h = GetNewLine(b3s, b3e);
                ShapePoint b3scenter = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, X_Base.PointY + b3h.Length / 2);
                graphicsShape.DrawString("B3=" + SetB3, drawFontBold, Brushes.Black, b3scenter, sf);



                ShapePoint b2s1 = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, W_Base.PointY);
                ShapePoint b2e1 = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, W_Base.PointY - SetB2);
                graphicsShape.DrawLine(pen, b2s1, b2e1);
                Line b2h1 = GetNewLine(b2s1, b2e1);
                ShapePoint b21scenter = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, W_Base.PointY - b2h1.Length / 2);
                ...
                ShapePoint h1scenter = GetNewCustomPoint(Y_Base.PointX + 10 * LineBoundArgument, SetPointCurrentValueY(E) + h1h.Length / 2);
                graphicsShape.DrawString("H2=" + SetCurrentSize(SetH2), drawFontBold, Brushes.Black, h1scenter, sf);
                ...
                ShapePoint h12scenter = GetNewCustomPoint(Z_Base.PointX + 10 * LineBoundArgument, SetPointCurrentValueY(H) - h1h2.Length / 2);
                ...
                ShapePoint hs = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, SetPointCurrentValueY(D));
                ShapePoint he = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, SetPointCurrentValueY(A));
                Line lh = GetNewLine(hs, he);
                ShapePoint hcenter = GetNewCustomPoint(Z_Base.PointX + 40 * LineBoundArgument, W_Base.PointY - (lh.Length / 2));
                ...
                ShapePoint ls12 = GetNewCustomPoint(SetPointCurrentValueX(G), Z_Base.PointY + 20 * LineBoundArgument);
                ShapePoint le12 = GetNewCustomPoint(SetPointCurrentValueX(H), Z_Base.PointY + 20 * LineBoundArgument);
                Line ll12 = GetNewLine(ls1, le1);
                ShapePoint l12center = GetNewCustomPoint(SetPointCurrentValueX(G) - (ll12.Length / 2), Z_Base.PointY + 10 * LineBoundArgument);
                ...
                ShapePoint ls = GetNewCustomPoint(SetPointCurrentValueX(B), W_Base.PointY + 40 * LineBoundArgument);
                ShapePoint le = GetNewCustomPoint(SetPointCurrentValueX(G), W_Base.PointY + 40 * LineBoundArgument);
                ...
                using (Pen pens11 = new Pen(Color.Red, SizeLineBoundArgument / 2))
                {
                    ShapePoint ls2 = GetNewCustomPoint(SetPointCurrentValueX(C), SetPointCurrentValueY(C));
                    ShapePoint le2 = GetNewCustomPoint(SetPointCurrentValueX(D), SetPointCurrentValueY(C));
                    Line ll2 = GetNewLine(ls2, le2);
                    ShapePoint l2center = GetNewCustomPoint(SetPointCurrentValueX(C) + (ll2.Length / 2), SetPointCurrentValueY(C) - 5 * LineBoundArgument);
                    graphicsShape.DrawLine(pens11, ls2, le2);
                    if (ll2.Length > 8)
                    {
                        sf.Alignment = StringAlignment.Center;
                        sf.LineAlignment = StringAlignment.Center;
                    ...
                    ShapePoint ls22 = GetNewCustomPoint(SetPointCurrentValueX(E), SetPointCurrentValueY(F));
                    ShapePoint le22 = GetNewCustomPoint(SetPointCurrentValueX(F), SetPointCurrentValueY(F));
                    Line ll22 = GetNewLine(ls22, le22);
                    ShapePoint l22center = GetNewCustomPoint(SetPointCurrentValueX(E) + (ll22.Length / 2), SetPointCurrentValueY(F) - 5 * LineBoundArgument);
                    graphicsShape.DrawLine(pens11, ls22, le22);
                    if (ll22.Length > 8)
                    {
                        sf.Alignment = StringAlignment.Center;
                        sf.LineAlignment = StringAlignment.Center;
                        graphicsShape.DrawString("L2=" + SetCurrentSize(SetL2), drawFontBold, Brushes.Black, l22center, sf);
                    }
                ...
                if (Area > 0.012)
                {
                    sf.Alignment = StringAlignment.Center;
                    sf.LineAlignment = StringAlignment.Center;
                    graphicsShape.DrawString("48", drawNumbertBold, Brushes.Black, mCustCenter, sf);
                }
                using (Pen pens = new Pen(Color.Blue, SizeLineBoundArgument / 2))
                {

                    var wer1 = GetNewCustomPoint(W_Base.PointX, SetPointCurrentValueY(D));
                    var wer2 = GetNewCustomPoint(SetPointCurrentValueX(C), X_Base.PointY);
                    var wer3 = GetNewCustomPoint(Z_Base.PointX, SetPointCurrentValueY(E));
                    var wer4 = GetNewCustomPoint(SetPointCurrentValueX(F), X_Base.PointY);
                    var wer5 = GetNewCustomPoint(Z_Base.PointX, SetPointCurrentValueY(H));
                    var wer6 = GetNewCustomPoint(SetPointCurrentValueX(G), Z_Base.PointY);
                    ...
                    using (pen1 = new Pen(Color.Black, ThiсknessArgument / 2))
                    {
                        ...
                        graphicsShape.DrawLine(pen1, wer4, SetPointCurrentType(F));
                        graphicsShape.DrawLine(pen1, wer5, SetPointCurrentType(H));
                        graphicsShape.DrawLine(pen1, wer6, SetPointCurrentType(G));
                        graphicsShape.DrawLine(pen1, wer7, SetPointCurrentType(A));
                        graphicsShape.DrawLine(pen1, wer8, SetPointCurrentType(B));
                    }
                    graphicsShape.DrawLine(pens, wer8, ls);
                    graphicsShape.DrawLine(pens, wer6, le);
                    ...
                    graphicsShape.DrawLine(pens, le12, SetPointCurrentType(H));
                    graphicsShape.DrawLine(pens, hs, wer3);
                    graphicsShape.DrawLine(pens, he, wer5);
                    graphicsShape.DrawLine(pens, h1s2, SetPointCurrentType(G));
                    graphicsShape.DrawLine(pens, h1s, SetPointCurrentType(F));
        ...
        public override double SetH1 { get => Math.Round(GetNewLine(GetNewCustomPoint(G.PointX, H.PointY), G).Length, 0); set => base.SetH1 = value; }
        public override double SetH2 { get => Math.Round(GetNewLine(GetNewCustomPoint(F.PointX, E.PointY), F).Length, 0); set => base.SetH2 = value; }
        public override double SetL1 { get => Math.Round(GetNewLine(GetNewCustomPoint(B.PointX, A.PointY), A).Length, 0); set => base.SetL1 = value; }
        public override double SetL2 { get => Math.Round(GetNewLine(GetNewCustomPoint(F.PointX, E.PointY), E).Length, 0); set => base.SetL2 = value; }
        ...
        public override double SetH2_t
        {
            get
            {
                ShapePoint d = GetNewCustomPoint(FCheck.PointX, ECheck.PointY);
                Line l = GetNewLine(d, FCheck);
                return Math.Round(l.Length, 0);
            }
        }
        ...
        protected override void SetLValue()
        {
            ...
            F.PointX = G.PointX;
            double diff = G.PointX - CurvePoint.PointX;
            H.PointX += diff;
            E.PointX += diff;
            ValidValue = false;
        }
        ...
        {
            base.SetH1Value();
            ShapePoint d = GetNewCustomPoint(G.PointX, H.PointY + 1);
            G.PointY = SetCurrentLineLength(d, G, _SetH1).PointY - 1;
            B.PointY = G.PointY;
            ValidValue = false;
        }
        ...
        {
            base.SetH2Value();
            ShapePoint d = GetNewCustomPoint(F.PointX, E.PointY);
            F.PointY = SetCurrentLineLength(d, F, _SetH2).PointY;
            C.PointY = F.PointY;
            ValidValue = false;
        }
        protected override void SetL1Value()
        {
            ShapePoint d = GetNewCustomPoint(B.PointX, A.PointY);
            A.PointX = SetCurrentLineLength(d, A, _SetL1).PointX;
            ShapePoint d1 = GetNewCustomPoint(G.PointX, H.PointY);
            H.PointX = SetCurrentLineLength(d1, H, _SetL1).PointX;
            ValidValue = false;
        }
        protected override void SetL2Value()
        {
            base.SetL2Value();
            ShapePoint d = GetNewCustomPoint(F.PointX, E.PointY);
            E.PointX = SetCurrentLineLength(d, E, _SetL2).PointX;
            ShapePoint d1 = GetNewCustomPoint(C.PointX, D.PointY);
            D.PointX = SetCurrentLineLength(d1, D, _SetL2).PointX;
            ValidValue = false;
        }
        ...
        public override bool CheckValidSize()
        {
            ...
            var value1 = (IsToothVector==true) ? _CheckCut2 : 0;
            var value2 = (IsToothVector==true) ? _CheckCut4 : 0;
            var value3 = (IsToothVector==true) ? _CheckCut6 : 0;
            var value4 = (IsToothVector == true) ? _CheckCut8 : 0;
            var width = _SetL + _SetB1*2  + value1 + value3;
            var height = _SetH + value2 + _SetB2+_SetB3 + value4;
            ...
            else if ((_SetH1 > 0) && _SetH1 > SetH-SetH2)
            {
                ValidValue = true;
                ValidateSetSizeMessage(Text: "Значение H1 не может быть больше H+H2");
            }
            else if ((_SetH2 > 0) && _SetH2 > SetH - SetH1)
            {
                ValidValue = true;
                ValidateSetSizeMessage(Text: "Значение H2 не может быть больше H+H1");
            }
            ...
            {
                ValidValue = true;
                ValidateSetSizeMessage(Text: "Значение L1 не может быть больше L+L2");
            }
            else if ((_SetL2 > 0) && _SetL2 > SetL - SetL1)
            {
                ValidValue = true;
                ValidateSetSizeMessage(Text: "Значение L2 не может быть больше L+L1");
            }
            else
