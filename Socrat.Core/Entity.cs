...
namespace Socrat.Core
{
    public class Entity : PropertyChangedBase, IEntity
    {
        ...
        public void CopyFieldsValues(Entity source, Entity destination)
        {
            var destProperties = destination.GetType().GetRuntimeFields();
            foreach (var sourceProperty in source.GetType().GetRuntimeFields().Where(x => x.FieldType.IsValueType))
            {
                foreach (var destProperty in destProperties)
                {
                    if (destProperty.Name == sourceProperty.Name
                        && destProperty.FieldType.GetTypeInfo()
                            .IsAssignableFrom(sourceProperty.FieldType.GetTypeInfo()))
                    {
