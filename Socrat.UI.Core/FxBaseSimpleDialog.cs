...
namespace Socrat.UI.Core
{
    public partial class FxBaseSimpleDialog : FxBaseForm, IEntityEditor
    {
        ...
        public new virtual bool Validate()
        {
            ...
            if (!res)
                XtraMessageBox.Show("Не заполнено обязательное поле/поля!", "Ошибка заполнения", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //break;
            return res;
        }
        ...
        /// <summary>
        /// Добавить верхний пункт главного меню
        /// </summary>
        /// <param name="caption">Заголовок</param>
        /// <returns>Ссылка на пункт</returns>
        public BarItemLink AddTopMenu(string caption)
        {
            if (_mainMenu == null)
            {
                InitMainMenu();
            }
            return barManager.MainMenu.AddItem(new BarSubItem {Caption = caption});
        }

        private void InitMainMenu()
        {
            _mainMenu = new Bar();
            barManager.Bars.Add(_mainMenu);
            barManager.MainMenu = _mainMenu;
            _mainMenu.BarAppearance.Hovered.BackColor = System.Drawing.SystemColors.ActiveCaption;
            _mainMenu.BarAppearance.Hovered.BackColor2 = System.Drawing.SystemColors.ActiveCaption;
            _mainMenu.BarAppearance.Hovered.Options.UseBackColor = true;
            _mainMenu.BarAppearance.Pressed.BackColor = System.Drawing.SystemColors.ActiveCaption;
            _mainMenu.BarAppearance.Pressed.BackColor2 = System.Drawing.SystemColors.ActiveCaption;
            _mainMenu.BarAppearance.Pressed.Options.UseBackColor = true;
            _mainMenu.BarName = "Главное меню";
            _mainMenu.DockCol = 0;
            _mainMenu.DockRow = 0;
            _mainMenu.DockStyle = DevExpress.XtraBars.BarDockStyle.Top;
            _mainMenu.OptionsBar.AllowCollapse = true;
            _mainMenu.OptionsBar.AllowQuickCustomization = false;
            _mainMenu.OptionsBar.AutoPopupMode = DevExpress.XtraBars.BarAutoPopupMode.All;
            _mainMenu.OptionsBar.DrawDragBorder = false;
            _mainMenu.OptionsBar.MultiLine = true;
            _mainMenu.OptionsBar.UseWholeRow = true;
            _mainMenu.Text = "Главное меню";
        }

        /// <summary>
        /// Добавить обычный пункт подменю
        /// </summary>
        /// <param name="barItemLink">Родительский пункт</param>
        /// <param name="caption">Заголовок</param>
        /// <param name="cmd">Команда</param>
        /// <returns>Ссылка на пункт</returns>
        public BarItemLink AddTopMenuItem(BarItemLink barItemLink, string caption, IMenuCommand cmd)
        {
            BarItemLink _itemLink = null;
            BarSubItem _subItem = barItemLink.Item as BarSubItem;
            if (_subItem != null)
            {
                BarButtonItem _barButtonItem = new BarButtonItem();
                _barButtonItem.Caption = caption;
                _barButtonItem.BindCommand(cmd);
                _itemLink = _subItem.AddItem(_barButtonItem);
            }
            return _itemLink;
        }

        /// <summary>
        /// Добавить чекбокс-пункт меню
        /// </summary>
        /// <param name="barItemLink">Родительский пункт</param>
        /// <param name="caption">Заголовок</param>
        /// <param name="cmd">Команда</param>
        /// <param name="groupIndex">индекс группы (в группе может быть выбран только один элемент)</param>
        /// <returns>Ссылка на пункт</returns>
        public BarItemLink AddTopMenuCheckItem(BarItemLink barItemLink, string caption, IMenuCommand cmd, bool initValue, int groupIndex = 0)
        {
            BarItemLink _itemLink = null;
            BarSubItem _subItem = barItemLink.Item as BarSubItem;
            if (_subItem != null)
            {
                BarCheckItem _barButtonItem = new BarCheckItem();
                _barButtonItem.Caption = caption;
                _barButtonItem.BindCommand(cmd);
                _itemLink = _subItem.AddItem(_barButtonItem);
                _barButtonItem.CheckStyle = BarCheckStyles.Standard;
                _barButtonItem.CheckBoxVisibility = CheckBoxVisibility.BeforeText;
                _barButtonItem.ButtonStyle = BarButtonStyle.Check;
                _barButtonItem.BindableChecked = initValue;
                if (groupIndex > 0)
                    _barButtonItem.GroupIndex = groupIndex;
            }
            return _itemLink;
        }

        public BarItemLink AddTopMenuSubItem(BarItemLink barItemLink, string caption)
        {
            BarItemLink _itemLink = null;
            BarSubItem _subItem = barItemLink.Item as BarSubItem;
            if (_subItem != null)
            {
                BarSubItem _barButtonItem = new BarSubItem();
                _barButtonItem.Caption = caption;
                _itemLink = _subItem.AddItem(_barButtonItem);
            }
            return _itemLink;
        ...
        public BarItemLink AddTopMenuButtonItem(BarItemLink barItemLink, string caption, IMenuCommand cmd)
        {
            BarItemLink _itemLink = null;
            BarSubItem _subItem = barItemLink.Item as BarSubItem;
            if (_subItem != null)
